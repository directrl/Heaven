...
namespace Coelum.Phoenix.Extensions.ImGui {
	...
	public class ImGuiController : IDisposable {
		...
		/// </summary>
		public ImGuiController(GL gl, IView view, IInputContext input, Action onConfigureIO) : this(
			gl, view, input, null, onConfigureIO) { }

		/// <summary>
		/// Constructs a new ImGuiController with font configuration and onConfigure Action.
		/// </summary>
		public unsafe ImGuiController(GL gl, IView view, IInputContext input,
		...
		public void Render() {
			if(_frameBegun) {
				var oldCtx = HImGui.GetCurrentContext();
				...
				_frameBegun = false;
				HImGui.Render();
				RenderImDrawData(HImGui.GetDrawData());

				if(oldCtx != Context) {
					HImGui.SetCurrentContext(oldCtx);
				}
			}
		}
		...
		private unsafe void SetupRenderState(ImDrawDataPtr drawDataPtr, int framebufferWidth,
		                                     int framebufferHeight) {
			// Setup render state: alpha-blending enabled, no face culling, no depth testing, scissor enabled, polygon fill
			_gl.Enable(GLEnum.Blend);
			_gl.BlendEquation(GLEnum.FuncAdd);
			_gl.BlendFuncSeparate(GLEnum.SrcAlpha, GLEnum.OneMinusSrcAlpha, GLEnum.One,
			...
			_gl.VertexAttribPointer((uint) _attribLocationVtxPos, 2, GLEnum.Float, false,
			                        (uint) sizeof(ImDrawVert), (void*) 0);
			_gl.VertexAttribPointer((uint) _attribLocationVtxUV, 2, GLEnum.Float, false,
			                        (uint) sizeof(ImDrawVert), (void*) 8);
			_gl.VertexAttribPointer((uint) _attribLocationVtxColor, 4, GLEnum.UnsignedByte, true,
		...
		private unsafe void RenderImDrawData(ImDrawDataPtr drawDataPtr) {
			...
			Vector2
				clipScale =
					drawDataPtr.FramebufferScale; // (1,1) unless using retina display which are often (2,2)
			...
			for(int n = 0; n < drawDataPtr.CmdListsCount; n++) {
				...
				_gl.BufferData(GLEnum.ElementArrayBuffer,
			...
			_gl.BlendFuncSeparate((GLEnum) lastBlendSrcRgb, (GLEnum) lastBlendDstRgb,
			...
			_gl.Scissor(lastScissorBox[0], lastScissorBox[1], (uint) lastScissorBox[2],
		...
		private unsafe void RecreateFontDeviceTexture() {
			...
			const int s = 64 * 1024; // TODO increase if needed
			
			byte* pixels = (byte*) Marshal.AllocHGlobal(s);
			Span<byte> pixelsS = new Span<byte>(pixels, s);
			pixelsS.Fill(0);
			
			io.Fonts.GetTexDataAsRGBA32(ref pixels, ref width, ref height,
			                            ref bytesPerPixel); // Load as RGBA 32-bit (75% of the memory is wasted, but default font is so small) because it is more likely to be compatible with user's existing shaders. If your ImTextureId represent a higher-level concept than just a GL texture id, consider calling GetTexDataAsAlpha8() instead to save on GPU memory.
			
			// Upload texture to graphics system
			_gl.GetInteger(GLEnum.TextureBinding2D, out int lastTexture);

			_fontTexture = new Texture(_gl, width, height, (IntPtr) pixels);
			_fontTexture.Bind();
			...
			// free unmanaged resources that have been uploaded to the GPU
