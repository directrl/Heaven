...
namespace Coelum.Graphics.Node {
	...
	public class SpatialNodeInstanced<TNode> : NodeBase, IRenderable, IShaderLoadable where TNode : SpatialNode {
		...
		public SpatialNodeInstanced(int capacity = 0) {
			_nodes = new(capacity);
			_models = new(capacity);
			_materials = new(capacity);
		}
		
		public SpatialNodeInstanced(List<TNode> nodes) {
			_nodes = nodes;
			...
			for(int i = 0; i < nodes.Count; i++) {
				...
				_materials[i] = obj.Model.Material;
			}
		}
		...
		public void Add(TNode obj) {
			...
			_materials.Add(obj.Model.Material);
		}
